...
def quat_to_omega_body(q, t_s, dt_min=1e-4, return_mask: bool = False):
    """
    四元数序列转体坐标系角速度
    ...
    for i in range(len(q) - 1):
        raw_dt = t_s[i + 1] - t_s[i]
        dt = max(raw_dt, dt_min)
        dR = R[i].T @ R[i + 1]
        w[i] = so3_log(dR) / dt  # rad/s（安全分母）
        valid[i] = (raw_dt > dt_min)
