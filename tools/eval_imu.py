...
if args.plots_dir:
    ...
    if args.compare_uncalibrated and args.calib_json:
        ...
        # Overlay hist of distributions: Pred (calibrated) log variance vs GT log error^2 (axes)
        for j,name in enumerate(["x","y","z"]):
            plt.figure(figsize=(6,4))
            plt.hist(P[...,j].ravel(),     bins=120, density=True, alpha=0.45, label="Pred")
        ...
        if args.also_overall:
            ...
            logE2_o = np.log(T_o + 1e-12)
            plt.figure(figsize=(6,4))
            plt.hist(P_o1.ravel(),    bins=120, density=True, alpha=0.45, label="Pred")
