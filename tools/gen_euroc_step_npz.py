...
try:
    ...
    from common.resample import interp_vec, central_diff as central_diff_safe
    from common.attitude import quat_to_omega_body as quat_to_omega_safe
    USE_NEW_GT = True
...
def process_seq(seq_root, out_dir, T=512, stride=256, g=9.80665, g_sign=-1, use_gt_bias=False,
    ...
    if USE_NEW_GT and state_gt_csv.exists():
        ...
        # 加速度（世界系）与掩码
        if USE_NEW_GT:
            a_w, valid_acc = central_diff_safe(v_i, t_imu, dt_min=1e-4, return_mask=True)
        else:
            a_w = central_diff(v_i, t_imu)
            valid_acc = np.ones_like(t_imu, dtype=bool)
        ...
        # 预测的体坐标角速度（由姿态增量）与掩码
        if USE_NEW_GT:
            w_pred_body, valid_w = quat_to_omega_safe(q_i, t_imu, dt_min=1e-4, return_mask=True)
        else:
            w_pred_body = quat_to_omega_body(q_i, t_imu)
            valid_w = np.ones_like(t_imu, dtype=bool)
        # 应用边缘mask
        mask_acc = valid_acc.copy()
        mask_gyr = valid_w.copy()
        if int(mask_edge) > 0:
            m = int(mask_edge); mask_acc[:m] = False; mask_acc[-m:] = False
        if int(gyr_mask_edge) > 0:
            m = int(gyr_mask_edge); mask_gyr[:m] = False; mask_gyr[-m:] = False
        # 残差
    ...
    np.savez(out,
        ...
        MASK_IMU=MASK,
        label_version="physics_v1"  # 工程规范：标注标签生成版本
    )
    # 也可写一个汇总 JSON
    meta = {
